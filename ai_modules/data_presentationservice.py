...
def presentData(mydb, db, queries, titles):
        ...
        if mapColumn is not None and total_length > 0:
            ...
            if restat['std'] is not None:
                ...
                # quantitative + quantitative = line
                    ...
                    chart_data.add_series(restat['std'].capitalize().replace('_',' '), [value for value in list(data[restat['std']]) if not math.isnan(value)])
                ...
                if (len(relevant) > 1 and typerin(firstRow[relevant[1]]) in ['float','int']) and validatePhone(firstRow[relevant[1]]):
                    chart_data = ChartData()
                ...
                elif len(relevant) > 1  and typerin(firstRow[relevant[0]]) in ['float','int'] and validatePhone(firstRow[relevant[0]]):
    ...
    for query in queries:
        ...
        if df.shape[0] > 1:
            ...
            row = json.dumps(df.iloc[0].to_json()).replace("\\\"","\"")
            gpt_prompt = '''Write a slide's text (single sentence) refering to the topic: ''' + subText[0] + '''\nThe slide's text must be regarding: ''' + row
            ...
            copiedTable.shapes[0].text = response['choices'][0]['message']['content'].replace("\"","")
            '''print(response)
            import numbers
            ...
            #if len(df.iloc[0]) > 1:            
            copiedTable.shapes[2].text = response #'With ' + ', '.join(str(x) for x in dict(df.iloc[0,1:]).values())'''
